...
class ChallongeTournament:
    ...
    def create_tournament(self):
        try:
            challonge.tournaments.create(
                name=self.tournament_name, url=self.url, game_name=self.game, tournament_type=self.tournament_type)
        ...
        for i in range(len(self.participant_names)):
            challonge.participants.create(
                tournament=self.url, name=self.participant_names[i], misc=self.participant_ids[i])
    ...
    def set_match_score(self, match_id, winner, scores):
        scores_csv = ','.join(
            ['-'.join([str(num) for num in score]) for score in scores])
        challonge.matches.update(
            tournament=self.url, match_id=match_id, winner_id=winner, scores_csv=scores_csv)
