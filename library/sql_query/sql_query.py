...
def initialize_connection():
    pool = MySQLConnectionPool(
        pool_name='primary_pool', pool_size=2, **connection)
    return pool
...
class SQLQuery():
    ...
    def insert_and_update(self, target_table, columns, row, key_list):
        try:
            ...
            data_str = str(tuple(row))[:-2] + \
                str(tuple(row))[-2:].replace(',', '')
            ...
            query_string = 'INSERT INTO %s (%s) VALUES %s' % (
                target_table, ', '.join(columns), data_str)
            if update_vals:
                query_string += ' ON DUPLICATE KEY UPDATE %s' % (
                    ', '.join(update_vals))
            print(query_string)
    ...
    def update_by_increment(self, target_table, columns, key_list, key_values, increment=1):
        try:
            ...
            key_values_tuples = [str(tuple(keys)) if len(keys) > 1 else str(
                tuple(keys)).replace(',', '') for keys in key_values]
            key_values_joined = ['%s IN %s' % (
                key_list[i], key_values_tuples[i]) for i in range(len(key_list))]

            query_string = 'UPDATE %s SET %s WHERE %s' % (
                target_table, ', '.join(update_cols), ' AND '.join(key_values_joined))
            print(query_string)
            cursor.execute(query_string)
    ...
    def delete_data(self, target_table, key_list, key_values):
        try:
            ...
            query_string = 'DELETE FROM %s WHERE %s' % (
                target_table, ' AND '.join(key_values_joined))
            print(query_string)
            cursor.execute(query_string)
            ...
            if kwargs:
                if 'condition' in list(kwargs.keys()):
                    ...
                    for i in range(len(kwargs[arg][0])):
                        vals = kwargs[arg][i+1]
                        vals = str(tuple(vals))[
                            :-2] + str(tuple(vals))[-2:].replace(',', '')
                        var_params.append(f'{kwargs[arg][0][i]} IN {vals}')
                    deep_query_string += ' WHERE %s' % (
                        ' AND '.join(var_params))
