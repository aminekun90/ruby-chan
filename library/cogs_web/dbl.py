...
class TopGG(commands.Cog):
    def __init__(self, bot):
        ...
        self.dblpy = dbl.DBLClient(self.bot, self.dbl_token, autopost=True, webhook_path='/dblwebhook',
                                   webhook_auth=self.webhook_auth_token, webhook_port=environ.get("PORT", 8000))
    ...
    async def on_dbl_vote(self, data):
        ...
        try:
            ...
            param_dict = {param.split('=')[0]: int(
                param.split('=')[1]) for param in query_string_params}
        ...
        try:
            if param_dict:
                guild_triggered_in = [
                    guild for guild in self.bot.guilds if guild.id == param_dict.get('guild')][0]
                ...
                embed = self.manager.create_embed(f'{user.name} has voted!', 'Thank you valued patron for supporting alice.'
                                                  ...
                                                  0xFFA500, 'attachment://user_awarded.png', [
                                                      'Award'], ['You got a one of a kind medallion!'],
