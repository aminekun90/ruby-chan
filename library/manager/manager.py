...
class Manager():
    ...
    def error_information(self, error_name, author):
        ...
        formatted_error_name = ' '.join(
            [word.capitalize() for word in error_name.split('_')])
    ...
    def create_embed(self, title, description, color, thumbnail, field_names, field_values, footer=[], url='', sign_embed=True):
        ...
        for i in range(len(field_names)):
            embed.add_field(name=field_names[i],
                            value=field_values[i], inline=False)
        return embed
...
def detect_help(expected_arg_count=0, split_by=None):
    ...
    def decorator(coro):
        ...
        async def wrapper(*args, **kwargs):
            ...
            command_args_str = ctx.message.content[len(
                curr_prefix+ctx.invoked_with):].strip()
            command_args = [
                command_args_str] if split_by == None else command_args_str.split(split_by)
            command_args = list(filter(None, command_args))
            manager = Manager()

            try:
                channel_occupied = channel.overwrites_for(
                    manager.get_role_by_name(channel.guild, 'alice tournament')).pair()[0]
...
def check_channel_perms(coro):
    ...
    async def wrapper(*args, **kwargs):
        ...
        if isinstance(ctx.channel, discord.channel.TextChannel):
            channel_perms = channel.overwrites_for(
                manager.get_role_by_name(channel.guild, 'alice dnd')).pair()[0]
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            if isinstance(ctx.channel, discord.channel.TextChannel):
                ...
                embed.add_field(
                    name='Guild', value=ctx.guild.name, inline=False)
                embed.add_field(
                    name='Channel', value=f'#{channel.name}', inline=False)
                await author.send(embed=embed)
                return None
