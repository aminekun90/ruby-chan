...
class Activation(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        # self.sql_query.insert_and_update(
        #     'guilds', ['guild_id', 'prefix'], [str(guild.id), '!'], [])
        ...
        for channel in guild.text_channels:
            if channel.permissions_for(guild.me).send_messages and not introduced:
                ...
                embed = self.manager.create_embed(hellos[random_hello],
                                                  ...
                                                  0x6FCE7B, 'attachment://alice.png', ['Vote'], [
                                                      '[Link](https://top.gg/bot/723813871881551932/vote)'],
    ...
    async def on_guild_remove(self, guild):
        disable_channel_role = self.manager.get_role_by_name(
            guild, 'alice dnd')
        ongoing_tournament_role = self.manager.get_role_by_name(
            guild, 'alice tournament')
    ...
    async def on_guild_channel_create(self, channel):
        disable_channel_role = self.manager.get_role_by_name(
            channel.guild, 'alice dnd')
        ongoing_tournament_role = self.manager.get_role_by_name(
            channel.guild, 'alice tournament')
