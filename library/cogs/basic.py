...
class Basic(commands.Cog):
    ...
    async def help_command(self, ctx):
        channel = ctx.channel
        help_field_names = [ctx.prefix + field_name for field_name in ['help', 'prefix',
                                                                       'enable', 'disable', 'open', 'ping', 'tournament', 'roundrobin', 'bracket', 'vote']]
        ...
        channel_occupied = channel.overwrites_for(
            self.manager.get_role_by_name(channel.guild, 'alice tournament')).pair()[0]
    ...
    async def set_prefix_command(self, ctx):
        ...
        desired_prefix = ctx.message.content[len(
            current_prefix+ctx.invoked_with):].strip()

        if len(desired_prefix) <= 5 and ' ' not in desired_prefix:
            # self.sql_query.insert_and_update('guilds', ['guild_id', 'prefix'], [
            #                                  str(channel.guild.id), desired_prefix], ['guild_id'])
    ...
    async def enable_channel_command(self, ctx):
        ...
        author = ctx.author
        current_prefix = ctx.prefix
        toggled_channel_id = ctx.message.content[len(
            current_prefix+ctx.invoked_with):].strip()
        channel_perms_role = self.manager.get_role_by_name(
            ctx.guild, 'alice dnd')
    ...
    async def open_channel_command(self, ctx):
        ...
        author = ctx.author
        current_prefix = ctx.prefix
        tournament_channel_id = ctx.message.content[len(
            current_prefix+ctx.invoked_with):].strip()
        channel_perms_role = self.manager.get_role_by_name(
        ...
        try:
            ...
            tournament_channel = ctx.guild.get_channel(
                int(tournament_channel_id))
