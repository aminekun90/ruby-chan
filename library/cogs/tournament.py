...
class Tournament(commands.Cog):
    ...
    async def user_reaction_on_message(self, desired_reactions, desired_user_ids, desired_messages, timeout=120, delete_on_timeout=False, channel_to_open=None):
        ...
        try:
            ...
            if channel_to_open:
                ongoing_tournament_role = self.manager.get_role_by_name(
                    # channel.guild
                    channel_to_open.guild, 'alice tournament')
    ...
    def match_result_string(self, participants_dict, match, winner_index):
        match_result = [f'**{participants_dict[participant]}**' if participant ==
                        match[winner_index] else participants_dict[participant] for participant in match]
        ...
        for i in range(len(rounds)):
            ...
            for j in range(len(matches)):
                ...
                embed = self.manager.create_embed('Match', f'{self.match_reactions[0]} <@{player1}>  vs.  <@{player2}> {self.match_reactions[1]}', self.round_robin_color, '',
                                                  ['Wins'], [
                                                      f'{participants_dict[player1]}: {wins_log[matches[j][0]]}\n{participants_dict[player2]}: {wins_log[matches[j][1]]}'],
            ...
            while not round_complete:
                ...
                round_complete = all(
                    len(set(match)) > 1 for match in match_votes)
            ...
            for j in range(len(matches)):
                ...
                matches_block.append(self.match_result_string(
                    participants_dict, match, winner_index))
            ...
            embed = self.manager.create_embed('Matches', f'<@{leader}> is in the lead with {wins_log[leader]}'
                                              f' {"wins" if wins_log[leader] > 1 else "win"}', self.round_robin_color, '', [
                                                  'Results'],
                                              ['\n'.join(matches_block)], footer=[footer_text+self.manager.current_time(), avatar_url])
            await channel.send(embed=embed)

        wins_log = {k: v for k, v in reversed(
            sorted(wins_log.items(), key=lambda item: item[1]))}
        rankings = ''
        place = 1
    ...
    async def bracket(self, channel, author, participants_dict, permitted_voters, ranked_participants, tournament_name, game_name, tournament_type):
        ...
        tournament_participants = dict([(participant['id'], participant['misc'][2:])
                                       for participant in tournament.fetch_participants()])
        ...
        while tournament.fetch_tournament()['state'] != 'awaiting_review':
            ...
            round_players = [[match["player1_id"], match["player2_id"]]
                             for match in matches]
            ...
            for i in range(len(matches)):
                ...
                tournament.set_match_score(
                    matches[i]['id'], round_players[i][winner_index], [score])

            leader = max(wins_log, key=wins_log.get)
            embed = self.manager.create_embed('Matches', f'<@{leader}> is on a streak with {wins_log[leader]}'
                                              f' {"wins" if wins_log[leader] > 1 else "win"}', self.bracket_color, '', [
                                                  'Results'],
                                              ['\n'.join(matches_block)], footer=[footer_text+self.manager.current_time(), avatar_url])
            await channel.send(embed=embed)
            round_count += 1

        tournament_winner = tournament_participants[round_players[i].pop(
            winner_index)]
        ...
        embed = self.manager.create_embed('Placings', f'{tournament_name} \u2022 {formatted_date}',
                                          self.bracket_color, 'attachment://winner.png', ['Winner', 'Runner-Up'], [
                                              f'**<@{tournament_winner}>**', f'<@{tournament_runnerup}>'],
    ...
    async def prompt_round_robin(self, ctx):
        ...
        command_args = [arg.strip() for arg in ctx.message.content[len(
            ctx.prefix+ctx.invoked_with):].split(',')]
        ...
        participants_dict = {
            str(member.id): member.display_name for member in participants}
        participant_ids = list(participants_dict.keys())
        random.shuffle(participant_ids)

        try:
            permitted_voters = [author.id] if (
                command_args[1] == 'true' or command_args[1] == 'yes') else participant_ids
        except:
            permitted_voters = participant_ids

        channel_occupied_role = self.manager.get_role_by_name(
            ctx.guild, 'alice tournament')
        channel_occupied = channel.overwrites_for(
            channel_occupied_role).pair()[0]
        ...
        if channel_occupied.read_messages and channel_occupied.send_messages:
            ...
            # self.sql_query.update_by_increment('guilds', ['round_robin_count'], [
            ...
            embed.add_field(
                name='Channel', value=f'#{channel.name}', inline=False)
    ...
    async def prompt_bracket(self, ctx):
        ...
        try:
            tournament_type = 'single elimination' if (command_args[2].lower(
            ) == 'se' or command_args[2].lower() == 'single elimination') else 'double elimination'
        except:
        ...
        try:
            permitted_voters = [author.id] if (command_args[3].lower(
            ) == 'true' or command_args[3].lower() == 'yes') else participant_ids
        except:
            try:
                permitted_voters = [author.id] if (command_args[2].lower(
                ) == 'true' or command_args[2].lower() == 'yes') else participant_ids
            except:
                permitted_voters = participant_ids
        ...
        if channel_occupied.read_messages and channel_occupied.send_messages:
            ...
            # self.sql_query.update_by_increment('guilds', ['bracket_count'], [
            #                                    'guild_id'], [[str(ctx.guild.id)]])
        else:
            embed = self.manager.error_information('channel_occupied', author)
            embed.add_field(name='Guild', value=ctx.guild.name, inline=False)
    ...
    async def prompt_tournament(self, ctx):
        ...
        if channel_occupied.read_messages and channel_occupied.send_messages:
            ...
            # self.sql_query.update_by_increment('guilds', ['tournament_count'], [
            #                                    'guild_id'], [[str(ctx.guild.id)]])
        else:
            embed = self.manager.error_information('channel_occupied', author)
            embed.add_field(name='Guild', value=ctx.guild.name, inline=False)
